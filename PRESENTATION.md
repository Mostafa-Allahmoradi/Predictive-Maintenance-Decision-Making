...
Framing as a Markov Decision Process  M = ⟨S, A, P, R, γ⟩
  ...
  S  →  ℝ²²   continuous state (22 sensor features)
  A  →  {0: Continue,  1: Maintain}
  P  →  unknown transition dynamics
  R  →  economic reward signal (see next slide)
...
```
STATE  s_t ∈ ℝ²²
  21 sensor channels (30-cycle trailing mean, Min-Max scaled to [0,1])
...
Key insight: −100 penalty is 5× the maintenance cost.
Failure must be strictly more expensive than prevention.
```
...
Double DQN fix (van Hasselt et al., 2016):
  ...
  Target network hard-copied from online every K = 200 gradient steps.
...
### Content
```
World model:   f_φ : ℝ^(22+2) → ℝ^22

  Input:   [s_t , one_hot(a_t)]   (state + action)
  Output:  Δ̂s = predicted residual

  Predicted next state:  ŝ_{t+1} = s_t + Δ̂s

  Loss:  L_world = (1/B) Σᵢ ‖f_φ(sᵢ, aᵢ) − (s'ᵢ − sᵢ)‖²

WHY residual and not absolute next state?

  Sensor channels:   Δs ~ 0.01  (tiny normalised changes)
  Cycle count:       Δt = +1    (exactly one per step)

  If we predicted s' directly, the cycle term would dominate
  the MSE by a factor of ~10,000 → sensors would be ignored.

  Residual prediction equalises gradient contribution across
  all 22 state dimensions.

Architecture:  Linear(24,256) → ReLU → Linear(256,256) → ReLU → Linear(256,22)
```
...
Policies: Double DQN  │  Fixed-Interval  │  Random
```
...
Full Project Stack
  ...
  MinMaxScaler        DynaQAgent
  EngineEpisode       PrioritizedReplayBuffer
  Window W=30         SumTree (O log n)
                      TensorBoard
                      tqdm progress bars
...
```
What we built:
  ✓ Full PdM RL system on NASA CMAPSS FD001
  ✓ Double DQN with corrected Bellman targets (no max bias)
  ✓ Dyna-Q with residual world model (fixed scale mismatch)
  ✓ Prioritised Experience Replay (SumTree, failure boost ×5)
